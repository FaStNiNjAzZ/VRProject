...
public class VRMovement : MonoBehaviour
{
    ...
    const float JOYSTICK_DEADZONE_THRESHOLD = 0.15f;
    const float PLAYER_MOVEMENT_ELEVATION_SPEED = 2.0f;
    ...
    void StrafeMovement(Vector2 left_Joystick_Value)
    {
        ...
        {
            transform.Translate(forwardDirection * PLAYER_MOVEMENT_ELEVATION_SPEED * Time.deltaTime, Space.World);
        }

        else if (left_Joystick_Value.y < -JOYSTICK_DEADZONE_THRESHOLD)
        {
            transform.Translate(-forwardDirection * PLAYER_MOVEMENT_ELEVATION_SPEED * Time.deltaTime);
        }

        if (left_Joystick_Value.x > JOYSTICK_DEADZONE_THRESHOLD)
        {
            transform.Translate(rightDirection * PLAYER_MOVEMENT_ELEVATION_SPEED * Time.deltaTime);
        }
