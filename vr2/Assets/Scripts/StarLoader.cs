...
{
    public TextAsset starData; // The CSV file should be attached here in the Inspector
    ...
    void Start()
    {
        ...
        for (int i = 1; i < data.Length - 1; i++)
        {
            ...
            float parallax = float.Parse(row[3]); // Parallax
            float magnitude = float.Parse(row[6]); // Magnitude

            // Avoid division by very small parallax values
            float distance = parallax > 0.001f ? 1.0f / parallax : 1000f;
